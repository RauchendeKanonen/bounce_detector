...
def load_npz(path: str):
    ...
    if symbols and len(symbols) > 1:
        ...
        symbols = ["DEFAULT"]
        meta["symbols"] = symbols  # Normalize for downstream
...
def make_contract(symbol: str, currency="USD", exchange="SMART") -> Contract:
    ...
    if c.symbol in ["XAUUSD", "XAGUSD"]:
        c.secType = "CMDTY"  # Commodities for gold/silver
    else:
...
if __name__ == "__main__":
    ...
    p_fetch.add_argument("--symbols", type=str, nargs='+', required=True, help="Symbols (space-separated, e.g., 'AAPL XAUUSD XAGUSD'). Auto-detects STK or CMDTY.")
    ...
    p_sample.add_argument("--symbols", type=str, nargs='*', help="Symbols to sample (default: all in file)")
    ...
    if args.cmd == "fetch":
        ...
        for sym in args.symbols:
            ...
            print(f"Fetched {quotes.size} quotes and {trades.size} trades for {sym}.")
        ...
        # Save with prefixes if multi, flat if single
        save_args = {}
        if len(args.symbols) > 1:
            ...
            del meta["symbols"]  # Omit for single to keep original structure
        ...
        selected_symbols = args.symbols if args.symbols else symbols  # Filter if specified

        # Compute global edges for alignment (union of time ranges)
        global_t0 = np.inf
        global_t1 = -np.inf
        for sym in selected_symbols:
            quotes = all_quotes[sym]
            trades = all_trades[sym]
            if quotes.size:
                global_t0 = min(global_t0, int(quotes["time"][0]))
                global_t1 = max(global_t1, int(quotes["time"][-1]))
            if trades.size:
                global_t1 = max(global_t1, int(trades["time"][-1]))

        if global_t0 == np.inf:
            raise ValueError("No data across selected symbols.")

        global_edges = make_bins(global_t0, global_t1 + args.dt_sec, args.dt_sec)

        # Sample each with global edges
        save_args = {}
        for sym in selected_symbols:
            print(f"Sampling {sym}...")
            sampled, info = sample_to_bins(all_quotes[sym], all_trades[sym], args.dt_sec, edges=global_edges)
        ...
        print(f"Sampled {len(global_edges)-1} aligned bins @ Δt={args.dt_sec}s -> {args.outfile}")
