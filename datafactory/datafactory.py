...
# ----------------------------------------------------------------------
# (imports unchanged – copy-paste from the previous version)
# ----------------------------------------------------------------------
import math
import time
import queue
...
# (parse_german_us_eastern, ib_utc_str_from_epoch unchanged)
...
def _paginate_ticks(
    ...
    req_timeout: float = 30.0,          # <-- NEW
):
    """
    Generic paginator with **per-request timeout**.
    If no data (or only an error) arrives within *req_timeout* seconds,
    the request is cancelled and re-issued from the same cursor.
    """
    cur = int(start_utc)
    end = int(end_utc)
    stalls = 0
    ...
    while cur < end:
        ...
        app.reqHistoricalTicks(
            ...
            endDateTime=ib_utc_str_from_epoch(end+1),
        ...
        while True:
            ...
            if time.time() - start_time > req_timeout:
                print(f"{what} request {reqId} TIMED OUT after {req_timeout}s – re-issuing")
                # Cancel the pending request (IB will ignore it)
                app.cancelHistoricalData(reqId)
                # Clean up the stale queue entry
                with app.lock:
                    app.result_queues.pop(reqId, None)
                    app.done_flags.pop(reqId, None)
                break   # break inner while → new request with same cur

            # ---- Normal exit (done flag + empty queue) ----
            ...
            if typ == "ERROR":
                _, code, msg = item
                if code in (162, 354, 10197):          # pacing / limit
...
# (fetch_historical_bidask / fetch_historical_trades unchanged except they now receive req_timeout)
...
# ---------------------- Persistence, Sampling, Contract ----------------------
# (load_npz, make_bins, sample_to_bins, make_contract unchanged – copy from previous version)
...
def sample_to_bins(
    ...
    # (unchanged – copy from previous version)
    if quotes.size == 0:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="IB historical ticks → NumPy → sampled features (US/Eastern input)")
    ...
    p_fetch.add_argument(
        ...
        help="Symbols (space-separated, e.g., 'AAPL XAUUSD XAGUSD'). Auto-detects STK or CMDTY.")
    p_fetch.add_argument("--currency", default="USD")
    p_fetch.add_argument("--exchange", default="SMART")
    p_fetch.add_argument(
        "--start-et", required=True,
        help="Start time in German format (US/Eastern), e.g. '28.10.2024 09:30:00' or '28.10.2024'")
    p_fetch.add_argument("--end-et", required=True,
                         help="End time in German format (US/Eastern)")
    p_fetch.add_argument("--rth-only", type=int, default=0,
                         help="1 = Regular Trading Hours only, 0 = all")
    p_fetch.add_argument("--req-timeout", type=float, default=30.0,
                         help="Seconds to wait for a single reqHistoricalTicks before re-issuing (default 30)")
    p_fetch.add_argument("--out", required=True, help="Output .npz path")

    # ---- sample ----
    p_sample = sub.add_parser("sample",
                              help="Load NPZ, sample to Δt bins, save sampled NPZ")
    ...
    p_sample.add_argument(
        ...
        help="Symbols to sample (default: all in file)")
    ...
    if args.cmd == "fetch":
        ...
        for sym in args.symbols:
            ...
            quotes = fetch_historical_bidask(
                app, contract, win, rth_only=args.rth_only,
                req_timeout=args.req_timeout)
            trades = fetch_historical_trades(
                app, contract, win, rth_only=args.rth_only,
                req_timeout=args.req_timeout)

            all_quotes[sym] = quotes
            all_trades[sym] = trades
            print(f"  → {quotes.size} quotes, {trades.size} trades")
    ...
    # ------------------------------------------------------------------
        ...
        if global_t0 == np.inf:
            raise ValueError("No data for the selected symbols.")
        edges = make_bins(global_t0, global_t1 + args.dt_sec, args.dt_sec)

        save_args = {}
        for sym in selected:
            print(f"Sampling {sym} …")
            sampled, info = sample_to_bins(
                all_quotes[sym], all_trades[sym], args.dt_sec, edges=edges)
            if len(selected) > 1:
        ...
        print(f"Sampled {len(edges)-1} aligned bins → {args.outfile}")
